...
class BaseExternalDbsource(models.Model):
    ...
    def onchange_conector(self):
        """ Assigning connection string to help fill.
        """
        conn_default_str = 'dsn=0.0.0.0:drive:/path/banco.gdb;user=sysdba;password=%s'
        if self.connector == 'fdb' and not self.conn_string:
            self.conn_string = conn_default_str
        else:
            if self.conn_string == conn_default_str:
                self.conn_string = False
